...
load("16S_Aeg_public/bennett/bennett_dada2.RData")

bennett_meta <- read_tsv("16S_Aeg_public/bennett.tsv")
bennett_meta <- as.data.frame(bennett_meta) %>%
  filter(
    Experiment %in% sample_names(otu_table(t(seqtab.nochim), taxa_are_rows = T))
  )
rownames(bennett_meta) <- bennett_meta$Experiment

ps <- phyloseq(
  otu_table(t(seqtab.nochim), taxa_are_rows = T),
  sample_data(bennett_meta),
  tax_table(taxid)
)
ps
...
p <- rel_abundance_clean |>
  ...
  facet_nested(
    ~ container_description + province + life_stage,
    scales = "free_x",
    space = "free",
    switch = "x",
...
grouped_relabund <- rel_abundance_clean |>
  group_by(OTU, life_stage, container_description, province) |>
...
grouped_relabund2 <- grouped_relabund |>
  group_by(life_stage, container_description, province) |>
  slice_max(mean, n = 15) |>
  pull(OTU) |>
  unique()
...
stage_list <- lapply(c("Adult", "Larvae"), function(stage) {
...
alluvial_grid <- df_alluvial |>
  ...
  ungroup() |>
  mutate(
    container_description = case_when(
      str_detect(container_description, "[Pp]lastic") ~ "plastic",
      container_description == "Tyre" ~ "tire",
...
ggsave("16S_Aeg_public/bennett_alluvial.pdf", dpi = 300, width = 7, height = 5)

save.image("16S_Aeg_public/bennett/bennett_dada2.RData")
