...
load("16S_Aeg_public/hernandez/hernandez_dada2.RData")

hernandez_meta <- read_tsv("16S_Aeg_public/hernandez.tsv")
hernandez_meta <- as.data.frame(hernandez_meta) %>%
  filter(
    Experiment %in% sample_names(otu_table(t(seqtab.nochim), taxa_are_rows = T))
  )
rownames(hernandez_meta) <- hernandez_meta$Experiment

ps <- phyloseq(
  otu_table(t(seqtab.nochim), taxa_are_rows = T),
  sample_data(hernandez_meta),
  tax_table(taxid)
)
ps
pq <- microbiome::add_refseq(ps)
...
grouped_relabund <- rel_abundance_clean |>
  group_by(OTU, life_stage, lat_lon) |>
  mutate(mean = mean(Abundance), median = median(Abundance)) |>
  ungroup() |>
  select(OTU, clean_Phylum, life_stage, lat_lon, mean, median) |>
  distinct()
...
stage_list <- lapply(c("eggs", "larves", "adult"), function(stage) {
  get_top_ASV_stage(grouped_relabund, stage)
})

df_alluvial <- grouped_relabund |>
  filter(OTU %in% grouped_relabund2) |>
  mutate(mean_larvae = if_else(life_stage == "larves", mean, NA_real_)) |>
  mutate(
...
alluvial_grid <- df_alluvial |>
  ...
  facet_grid2(vars(lat_lon), axes = "all", scales = "free_y") +
...
ggsave(
  "16S_Aeg_public/hernandez_alluvial.pdf",
  dpi = 300,
  width = 7,
  height = 10
)

save.image("16S_Aeg_public/hernandez/hernandez_dada2.RData")
